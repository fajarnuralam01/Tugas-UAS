...
with tab1:
    ...
    st.pyplot(fig_trend)
    with st.expander("Lihat Analisis Adrian (Q1)"):
        st.write("Terlihat terjadi peningkatan signifikan jumlah penyewaan dari tahun 2011 ke 2012. Ini menunjukkan bisnis sedang tumbuh.")
    ...
    with col_a:
        ...
        st.pyplot(fig_month)
        with st.expander("Lihat Analisis Joao (Q11)"):
            st.write("Penyewaan memuncak di pertengahan tahun (Juni-September) dan turun di awal/akhir tahun (Januari/Desember).")
    ...
    with col_b:
        ...
        st.pyplot(fig_day)
        with st.expander("Lihat Analisis Rafi (Q8)"):
            st.write("Rata-rata penyewaan relatif stabil sepanjang minggu, namun sedikit lebih tinggi pada hari Jumat (5).")
...
with tab2:
    ...
    ax_hour.grid(True)
    st.pyplot(fig_hour)
    with st.expander("Lihat Analisis Rifky (Q3)"):
        st.write("Pola jam menunjukkan bimodal distribution: puncak terjadi pada jam 8 pagi (berangkat kerja) dan jam 5-6 sore (pulang kerja).")
    ...
    st.pyplot(fig_work)
    with st.expander("Lihat Analisis Rifky (Q4)"):
        st.write("Secara median, jumlah penyewaan pada hari kerja (Working Day) sedikit lebih tinggi dibandingkan hari libur, mengindikasikan penggunaan sepeda banyak untuk komuter.")
...
with tab3:
    ...
    with col_c:
        ...
        st.pyplot(fig_sea)
        with st.expander("Lihat Analisis Rizki (Q5)"):
            st.write("Musim Gugur (Fall) memiliki rata-rata penyewaan tertinggi, diikuti Musim Panas. Musim Semi (Spring) paling rendah.")
        ...
        st.pyplot(fig_temp)
        with st.expander("Lihat Analisis Adrian (Q2)"):
            st.write("Terdapat korelasi positif yang kuat; semakin hangat suhu, semakin banyak orang menyewa sepeda. Namun, pada suhu yang ekstrem (sangat panas), tren mungkin sedikit menurun.")
    ...
    with col_d:
        ...
        st.pyplot(fig_weath)
        with st.expander("Lihat Analisis Rafi (Q7)"):
            st.write("Cuaca buruk (Hujan/Salju) menurunkan penyewaan secara drastis. Pada kondisi cuaca kategori 3 (Light Snow/Rain), penyewaan jatuh signifikan.")
        
        # Q6 & Q10 (Rizki & Fajar)
        st.subheader("4. Kelembaban & Angin")
        tab_sub1, tab_sub2 = st.tabs(["Kelembaban (Rizki)", "Kecepatan Angin (Fajar)"])
        
        with tab_sub1:
            fig_hum, ax_hum = plt.subplots()
            sns.scatterplot(data=main_df, x='hum', y='cnt', color='teal', alpha=0.5, ax=ax_hum)
            st.pyplot(fig_hum)
            st.write("**Analisis Rizki (Q6):** Kelembaban memiliki korelasi negatif lemah. Jika terlalu lembab, penyewaan cenderung turun.")
            
        with tab_sub2:
            fig_wind, ax_wind = plt.subplots()
            sns.scatterplot(data=main_df, x='windspeed', y='cnt', color='purple', alpha=0.5, ax=ax_wind)
            st.pyplot(fig_wind)
            st.write("**Analisis Fajar (Q10):** Kecepatan angin memiliki korelasi negatif. Semakin kencang angin, semakin sedikit orang bersepeda demi kenyamanan.")
...
with tab4:
    ...
    with col_e:
        ...
        st.pyplot(fig_pie)
        with st.expander("Lihat Analisis Fajar (Q9)"):
            st.write("Pengguna terdaftar (Registered) mendominasi penggunaan sepeda (>80%), menunjukkan loyalitas pelanggan atau penggunaan rutin (seperti bekerja).")
    ...
    with col_f:
        ...
        ax_user.grid(True)
        st.pyplot(fig_user)
        with st.expander("Lihat Analisis Joao (Q12)"):
            st.write("Pengguna Registered memiliki dua puncak (8 pagi dan 5 sore) yang menandakan aktivitas komuter. Pengguna Casual memiliki satu puncak landai di siang hari (wisata/rekreasi).")

# === TAB 5: DATA MINING (CLUSTERING) ===
with tab5:
    st.header("🔍 Advanced Analysis: Clustering")
    st.markdown("Mengelompokkan hari berdasarkan kondisi cuaca (Temperatur, Kelembaban, Angin).")
    ...
    st.info("""
    **💡 Interpretasi Bisnis:**
    - **Cluster dengan titik besar (Kuning/Terang):** Hari dengan jumlah penyewaan tinggi. Biasanya cuaca hangat dan kelembaban sedang.
    - **Cluster dengan titik kecil (Gelap):** Hari dengan jumlah penyewaan rendah. Biasanya saat hujan atau kelembaban ekstrem.
    """)

# === TAB 6: FORECASTING (PREDIKSI) ===
with tab6:
    st.header("🔮 Forecasting: Prediksi Masa Depan")
    st.markdown("Prediksi tren penyewaan sepeda untuk 30 hari ke depan menggunakan teknik **Time Series Analysis** (Simple Moving Average).")
    
    # Hitung Moving Average (MA)
    window_size = st.slider("Rentang Waktu Prediksi (Hari):", 7, 60, 30)
    main_df['MA'] = main_df['cnt'].rolling(window=window_size).mean()
    
    fig_forecast, ax_forecast = plt.subplots(figsize=(14, 6))
    ax_forecast.plot(main_df['dteday'], main_df['cnt'], label='Data Aktual', color='lightgray', alpha=0.5)
    ax_forecast.plot(main_df['dteday'], main_df['MA'], label=f'Prediksi (Moving Average {window_size} Hari)', color='blue', linewidth=2)
    ax_forecast.set_title("Analisis & Prediksi Tren Penyewaan")
    ax_forecast.legend()
    st.pyplot(fig_forecast)
    
    st.success(f"Garis Biru menunjukkan tren prediksi yang diperhalus, membantu manajemen melihat arah bisnis ke depan tanpa gangguan fluktuasi harian.")

# === TAB 7: GEOANALYSIS (PETA) ===
with tab7:
    st.header("🗺️ Geoanalysis: Operational Map")
    st.markdown("""
    **Catatan:** Dataset ini bersifat agregat dan tidak memiliki koordinat GPS per transaksi. 
    Peta di bawah ini adalah visualisasi lokasi stasiun utama **Capital Bikeshare di Washington D.C.** sebagai konteks wilayah operasional.
    """)
    
    # Data Dummy Stasiun Utama di Washington DC (Latitude/Longitude Asli)
    stations_data = pd.DataFrame({
        'lat': [38.9072, 38.8977, 38.8898, 38.8977, 38.8913, 38.9096, 38.9200, 38.9050, 38.8814, 38.9000],
        'lon': [-77.0369, -77.0365, -77.0091, -77.0058, -77.0502, -77.0434, -77.0400, -77.0600, -77.0120, -77.0500],
        'name': ['DC Center', 'White House', 'Capitol', 'Union Station', 'Lincoln Memorial', 'Dupont Circle', 'Adams Morgan', 'Georgetown', 'Nationals Park', 'Foggy Bottom']
    })
    
    st.map(stations_data, zoom=12)
    
    st.info("Peta ini menunjukkan sebaran titik strategis di mana layanan Bike Sharing beroperasi secara intensif.")
